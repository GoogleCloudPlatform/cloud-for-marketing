...
'use strict';
const {
  FirestoreAccessBase: {DataSource, FirestoreAccessBase, Entity},
  NativeModeAccess,
  DatastoreModeAccess,
} = require('nodejs-common');
const {TaskStatus, TentaclesTaskBase} = require('./tentacles_task_base.js');
 ...
 */
class TentaclesTaskOnFirestore extends TentaclesTaskBase {
  /**
  ...
  constructor(dataSource) {
    ...
    /** @type {!FirestoreAccessBase} */ this.taskAccessObject;
    switch (dataSource) {
      case DataSource.FIRESTORE:
        this.taskAccessObject = new NativeModeAccess('tentacles/database/Task');
        break;
      case DataSource.DATASTORE:
        this.taskAccessObject = new DatastoreModeAccess('tentacles', 'Task');
        break;
    }
  }
   ...
   */
  parseTaskId_(id) {
    if (this.dataSource === DataSource.DATASTORE && typeof id === 'string') {
      return parseInt(id);
    }
    return id;
  }
  ...
  checkedSave_(id, fn) {
    ...
    return this.taskAccessObject.getObject(id).then((task) => {
      if (!task) {
      ...
      return Promise.resolve(fn(task))
          .then((task) => {
            ...
            return this.taskAccessObject.saveObject(task, id).then(() => true);
          })
          .catch((error) => {
            console.error(error);
            return false;
  ...
  create(entitySkeleton) {
    ...
    return this.taskAccessObject.saveObject(entity);
  }

  /** @override */
  update(id, data) {
    /** @type {!GetUpdatedTask} */
    const addMessageId = (task) => {
