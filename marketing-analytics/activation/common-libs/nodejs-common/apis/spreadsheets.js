...
class Spreadsheets {
  ...
  constructor(spreadsheetId) {
    ...
    const authClient = new AuthClient(SHEETS_API_SCOPES);
    /** @const {!Promise<!Sheets>} */
  ...
  getSheetId(sheetName) {
    ...
    return this.instance.then((sheets) => {
      return sheets.spreadsheets.get(request).then((response) => {
  ...
  reshape(sheetName, targetRows, targetColumns) {
    ...
    return this.instance.then((sheets) => {
      return sheets.spreadsheets.get(request)
          .then((response) => {
            const sheet = response.data.sheets[0];
            ...
            if (requests.resource.requests.length > 0) {
              return sheets.spreadsheets.batchUpdate(requests).then(
                  (response) => {
                    const data = response.data;
...
exports.ParseDataRequest = ParseDataRequest;
